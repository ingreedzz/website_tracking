  ...
  try {
    ...
    try {
      ...
      const histObjExtended = {
        ...
        new_status: status,
        // Keep legacy `changed_by` as the UUID (legacy column is typed uuid)
        changed_by: changer.users_id || userId,
      ...
      // Debug: log full response from Supabase insert
      console.log(`[REQ:${requestId}] [ORDER-STATUS] Extended insert response`, {
        data: r && r.data ? r.data : null,
        error: r && r.error ? { message: r.error.message, details: r.error.details } : null
      });
      histIns = r && r.data ? r.data : null;
      if (r && r.error) {
        ...
        const histObj = {
          ...
          old_status: oldStatus,
          new_status: status,
          // Legacy column expects a UUID — use the user's id here
          changed_by: changer.users_id || userId,
          note: note || null
        };
        ...
        console.log(`[REQ:${requestId}] [ORDER-STATUS] Fallback insert response`, {
          data: r2 && r2.data ? r2.data : null,
          error: r2 && r2.error ? { message: r2.error.message, details: r2.error.details } : null
        });
        histIns = r2 && r2.data ? r2.data : null;
        if (r2 && r2.error) {
